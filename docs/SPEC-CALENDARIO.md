...
> **Estado**: fase 1 IMPLEMENTADA el 2026-08-20 (SOLO Google — Microsoft pospuesto por decisión de Juan; §2 fase 2 queda en espera). Código: `worker/calendar.js`, migración `0012_calendario.sql`, tool use en `worker/app.js` (`callAnthropicRaw`/`runToolLoop`), híbrido en `handleTwilio`, pestaña Calendario en el panel, 8 bloques de tests (suite 87/87). PENDIENTE para activarla: credenciales del proyecto `velai-calendar` de Google Cloud (`GOOGLE_OAUTH_CLIENT_ID` en wrangler.toml + secret `GOOGLE_OAUTH_CLIENT_SECRET`) y la verificación de la app por Google (en Testing los refresh caducan a 7 días). Con la var vacía la función queda APAGADA (el chat funciona sin citas). El picker de calendarios (§6) se dejó como campo de texto `calendar_id` para no ampliar el scope OAuth; la UI de citas para el rol cliente queda para después (el endpoint ya lo permite).
> **Alcance original**: conexión de Google Calendar / Microsoft 365 por tenant, tool use en `callAnthropic`, agendado desde cualquier canal ya desplegado (web, WhatsApp, Messenger). Telegram-cliente e Instagram quedan explícitamente en fase aparte (§9).
