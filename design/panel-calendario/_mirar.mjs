...
for(const ab of canvas.artboards){
  ...
  const m=await p.evaluate(()=>{const m=document.querySelector('main')||document.querySelector('.root');const r=m.getBoundingClientRect();
    const hijos=[...m.children].map(c=>c.getBoundingClientRect().bottom);const fin=Math.max(r.top+40,...hijos);
    return {alto:Math.ceil(fin+28),ancho:Math.ceil(document.querySelector('.root').getBoundingClientRect().width)};});
