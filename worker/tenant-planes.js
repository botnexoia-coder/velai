...
import { PLANES, MODULOS, esPlan, modulosDe, canalesOcupados } from './planes.js';
...
export async function assertPlanChannelLimit(env, tenantId, plan, cambio = {}, previous = null) {
  if (!esPlan(plan)) throw new HttpError(400, 'invalid_plan');
  if (PLANES[plan].canales === Infinity) return;
  ...
  if (canalesOcupados({ ...tenant, ...cambio }, filas).length > PLANES[plan].canales) throw new HttpError(409, 'plan_channel_limit');
}
