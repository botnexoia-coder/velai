...
function semaforo(t){if(!t.active&&String(t.channel_address).startsWith('pending:'))return '<span class="muted">prospecto</span>';
 if(String(t.channel_address).startsWith('web:')){const f=[];if(t.prompt_len<200)f.push('contexto corto');if(!t.has_team&&!t.has_telegram)f.push('sin canal de aviso');return '<span class="pill">solo web</span>'+(f.length?' <span class="error">'+esc(f.join(' · '))+'</span>':' ✓')}
 const f=[];if(!t.has_template)f.push('sin plantilla');if(!t.has_team)f.push('sin equipo');if(t.prompt_len<200)f.push('contexto corto');if(t.has_subaccount&&!t.has_twilio_token)f.push('sin token');if(t.has_subaccount&&!t.has_from)f.push('sin From');if(t.meta_partner_status==='pendiente'&&t.has_subaccount)f.push('socio pendiente');return f.length?'<span class="error">'+esc(f.join(' · '))+'</span>':'✓'}
